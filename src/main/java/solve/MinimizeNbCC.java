...
public class MinimizeNbCC {
    ...
    public static void main(String[] args) throws IOException {
        ...
        // Borendy //
        BaseProblemBorendy problemBorendy = new BaseProblemBorendy("nbCCBorendy");

        Model modelBorendy = problemBorendy.reserveModel.getChocoModel();

        int nbCCForest = problemBorendy.forest.getSetVar().getGLB().getNbCC();

        IntVar nbCCBorendy = problemBorendy.potentialForest.getNbCC();
        ...
        System.out.println("NbCC initial = " + nbCCForest);

        for (int a = 90; a <= 110; a++) {
            Constraint area = modelBorendy.arithm(problemBorendy.minReforestAreaBorendy, "=", a);
            modelBorendy.post(area);
            Solution s = solverBorendy.findOptimalSolution(nbCCBorendy,false);
            borendyFront.put(s.getIntVal(problemBorendy.minReforestAreaBorendy), s.getIntVal(nbCCBorendy));
            System.out.println(Arrays.toString(new int[] {s.getIntVal(problemBorendy.minReforestAreaBorendy), s.getIntVal(nbCCBorendy)}));
            modelBorendy.unpost(area);
            solverBorendy.reset();
        }

        System.out.println("minArea,nbCC");
        ...
        // Unia //
        BaseProblemUnia problemUnia = new BaseProblemUnia("nbCCUnia");

        Model modelUnia = problemUnia.reserveModel.getChocoModel();

        IntVar nbCCUnia = problemUnia.potentialForest.getNbCC();
        ...
        for (int a = 90; a <= 110; a++) {
            ...
            modelUnia.post(area);
            Solution s = solverUnia.findOptimalSolution(nbCCUnia,false);
            uniaFront.put(s.getIntVal(problemUnia.minReforestAreaUnia), s.getIntVal(nbCCUnia));
            System.out.println(Arrays.toString(new int[] {s.getIntVal(problemUnia.minReforestAreaUnia), s.getIntVal(nbCCUnia)}));
        ...
        IntVar valBorendy = model.intVar("nbCCBorendy",0, 1000);
        IntVar valUnia = model.intVar("nbCCUnia",0, 1000);
        ...
        IntVar total = model.intVar("totalNbCC", 0, 1000);
        model.arithm(model.intOffsetView(valBorendy, -nbCCForest), "+", valUnia, "=", total).post();
        ...
        solver.plugMonitor((IMonitorSolution) () -> {
            ...
            System.out.println("NbCC Borendy = " + valBorendy.getValue());
            System.out.println("Area Unia = " + areaUnia.getValue());
            System.out.println("NbCC Unia = " + valUnia.getValue());
            System.out.println("Total nbCC = " + total.getValue());
        });
        solver.showStatistics();
        List<Solution> allocs = solver.findAllOptimalSolutions(total, false);
        ...
        for (Integer[] alloc : optimalAllocations) {
            ...
            Constraint valB = modelBorendy.arithm(nbCCBorendy, "=", borendyFront.get(alloc[0]));
            areaB.post();
            valB.post();
            ...
            Constraint valU = modelUnia.arithm(nbCCUnia, "=", uniaFront.get(alloc[1]));
            areaU.post();
            valU.post();
